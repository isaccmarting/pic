...
// 视角和坐标轴夹角的余弦 
GLfloat cos_xViewAngle = 0.0f, cos_yViewAngle = 0.0f; 
// 视野的半径 
GLfloat r = 7.0f; 
// 临时观察点，以center为中心 
GLfloat center_t[] = {0.0f, 0.0f, 0.0f}; 
...
void onMouseMove(int x, int y)
{
	...
	GLfloat xRange = PI / 180 * 30; 
	GLfloat yRange = PI / 180 * 23; 
	GLfloat xViewAngle; 
	GLfloat yViewAngle; 
	// 更新当前的视角与坐标轴的夹角 
	xViewAngle = (2.0f * x / wWidth - 1) * xRange + PI/2; 
	yViewAngle = (2.0f * y / wHeight - 1) * yRange + PI/2; 
	// 保存余弦值 
	cos_xViewAngle = cos(xViewAngle); 
	cos_yViewAngle = cos(yViewAngle); 
	// fprintf(stderr, "%lf %lf\n", 2.0f * x / wWidth - 1, 2.0f * y / wHeight - 1); 
	// fprintf(stderr, "%lf %lf %lf\n", center_t[0], center_t[1], center_t[2]); 
}
...
void redraw()
{
	...
	// 计算的临时观察点坐标 
	center_t[0] = eye[0] - r * cos_xViewAngle; 
	center_t[1] = eye[1] + r * cos_yViewAngle; 
	center_t[2] = eye[2] - r * sqrt(1 - cos_xViewAngle * cos_xViewAngle - cos_yViewAngle * cos_yViewAngle); 
	// 视点、观察坐标和观察正方向
